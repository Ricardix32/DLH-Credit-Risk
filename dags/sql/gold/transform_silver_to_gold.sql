...
INSERT INTO gold.fact_creditos (
    dd_id_solicitud, sk_cliente, sk_producto, sk_tiempo, sk_proxy_documental,
...
SELECT 
    s.bk_id_solicitud AS dd_id_solicitud, -- Traemos el ID como Dimensión Degenerada
    c.sk_cliente,
    p.sk_producto,
    t.sk_tiempo,
    1 AS sk_proxy_documental, 
    s.amt_credit,
    s.amt_income_total,
    s.amt_annuity,
    s.days_employed,
...
-- EL CAMBIO MAESTRO: Si la solicitud ya existe, solo actualizamos montos (UPSERT)
ON CONFLICT (dd_id_solicitud) DO UPDATE SET
    sk_tiempo = EXCLUDED.sk_tiempo, -- Actualizamos al último día en que se evaluó el estado

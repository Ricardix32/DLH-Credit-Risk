-- =================================================================
-- PASO 3: FUSIÓN DE METADATA IDP Y HISTÓRICO EN CAPA SILVER
-- =================================================================

-- 1. Rama A: Extraer el registro Base (Kaggle) más reciente por cliente
WITH solicitudes_base_reciente AS (
    SELECT 
        bk_id_solicitud,
        datos_origen_raw,
        via_airflow_run_id,
    ...
    WHERE datos_origen_raw->>'IDP_METADATA' IS NULL
),
solicitudes_base AS (
    SELECT
        ...
        via_airflow_run_id
    FROM solicitudes_base_reciente
    WHERE rn = 1
),

-- 2. Rama B: Extraer el documento IDP (OCR) más reciente por cliente
documentos_idp_reciente AS (
    SELECT 
        bk_id_solicitud,
        datos_origen_raw,
        via_airflow_run_id,
        ROW_NUMBER() OVER (PARTITION BY bk_id_solicitud ORDER BY fecha_ingestion DESC) as rn
    FROM bronze.raw_application
    WHERE datos_origen_raw->>'IDP_METADATA' IS NOT NULL
),
documentos_idp AS (
    SELECT
        bk_id_solicitud::INTEGER as id_solicitud,
        ...
        via_airflow_run_id
    FROM documentos_idp_reciente
    WHERE rn = 1
),

-- 3. Fusión (Merge) priorizando la información estructurada por la IA
fusion_features AS (
    SELECT
        b.id_solicitud,
        b.target,
...
-- 4. Carga final al Feature Store (Silver) con UPSERT
...
-- =================================================================
-- PASO 5: REGISTRO DE AUDITORÍA Y LINAJE
-- =================================================================
...
SELECT 
    ...
    (SELECT COUNT(*) FROM silver.cleaned_application WHERE via_airflow_run_id = '{{ run_id }}'),
    'EXITO',
